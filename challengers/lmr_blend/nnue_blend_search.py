...
from nnue_blend_incremental import (
...
# Keep these public so the main module can select controlled blend variants.
NEURAL_BLEND_NUMERATOR: Final = 1
CLASSICAL_BLEND_NUMERATOR: Final = 7
BLEND_DENOMINATOR: Final = 8
NEURAL_CLAMP_CP: Final = 200
MIN_BLEND_PIECES: Final = 17
...
def evaluate_blended(
    ...
    """Blend NNUE with classical evaluation from the side-to-move view."""
    classical = evaluate_classical(board, state)
    occupied = 0
    for piece in board:
        if piece != 0:
            occupied += 1
    if occupied < MIN_BLEND_PIECES:
        return classical
    neural = evaluate_nnue(state, white_accumulator, black_accumulator)
    neural = max(classical - NEURAL_CLAMP_CP, min(classical + NEURAL_CLAMP_CP, neural))
    total = CLASSICAL_BLEND_NUMERATOR * classical + NEURAL_BLEND_NUMERATOR * neural
    if total >= 0:
        return (total + BLEND_DENOMINATOR // 2) // BLEND_DENOMINATOR
    return -((-total + BLEND_DENOMINATOR // 2) // BLEND_DENOMINATOR)
...
def _quiescence(
    ...
    if qdepth >= MAX_QDEPTH:
        return evaluate_blended(board, state, white_accumulator, black_accumulator)
    if not in_check:
        stand_pat = evaluate_blended(board, state, white_accumulator, black_accumulator)
        if stand_pat >= beta:
            return stand_pat
        if stand_pat > alpha:
            alpha = stand_pat
