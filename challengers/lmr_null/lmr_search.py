...
def _negamax(
    ...
    # A hypothetical skipped turn is only a pruning probe, never a legal move.
    # Restrict it to narrow-window middlegames; sparse endings are vulnerable
    # to zugzwang (where being required to move makes the position worse).
    if (
        allow_null
        and depth >= 4
        and beta - alpha == 1
        and abs(beta) < MATE - MAX_SEARCH_PLY
        and state[HALFMOVE] < 80
        and not _in_check(board, state)
        and evaluate(board, state) >= beta
    ):
        ours = theirs = 0
        for piece in board:
            if PAWN < abs(piece) < KING:
                if piece * state[SIDE] > 0:
                    ours += 1
                else:
                    theirs += 1
        if ours >= 2 and theirs >= 2 and has_legal_move(board, state):
            saved_state = state.copy()
            state[HASH_KEY] ^= ZOBRIST_SIDE
            if state[EP_SQUARE] >= 0:
                state[HASH_KEY] ^= ZOBRIST_EP[int(state[EP_SQUARE])]
            state[SIDE] = -state[SIDE]
            state[EP_SQUARE] = -1
            # Synthetic turns must not contaminate actual repetition history.
            # Actual move clocks are retained; this is not a played move.
            null_path = np.zeros(MAX_SEARCH_PLY, dtype=np.int64)
            null_score = -_negamax(
                board,
                state,
                depth - 3,
            ...
            if stats[1] != 0:
                return 0
            if beta <= null_score < MATE - MAX_SEARCH_PLY:
                return beta
