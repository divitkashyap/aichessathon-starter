...
def evaluate(board: np.ndarray, state: np.ndarray) -> int:
    white_king_square = -1
    black_king_square = -1
    white_queen_present = False
    black_queen_present = False
    ...
    for square in range(64):
        ...
        if piece_type == chess.PAWN:
            pawn_side = 1 if piece > 0 else -1
            king_square = white_king_square if pawn_side > 0 else black_king_square
            opponent_queen_present = black_queen_present if pawn_side > 0 else white_queen_present
            if king_square >= 0 and opponent_queen_present:
                king_file = king_square & 7
                king_relative_rank = (king_square if pawn_side > 0 else king_square ^ 56) >> 3
                pawn_file = square & 7
                pawn_relative_rank = (square if pawn_side > 0 else square ^ 56) >> 3
                if (
                    (king_file <= 2 or king_file >= 5)
                    and king_relative_rank <= 1
                    and abs(pawn_file - king_file) <= 1
                    and pawn_relative_rank > 1
                    and pawn_relative_rank > king_relative_rank
                ):
                    middlegame -= pawn_side * (pawn_relative_rank - 1) * 9
