"""Local harness entrypoint for the isolated LMR pawn-evaluation challenger."""

import chess
from pawn_core import HASH_KEY, position_from_fen
from pawn_search import search_timed, warm_up

warm_up()
BUILD_ID = "einsteinanium-lmr-pawns-challenger"
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    try:
        ...
        print(f"lmr pawn search fallback: {type(error).__name__}: {error}")
        _remember_after(board, fallback)
        return fallback
    if chess.Move.from_uci(result.move) not in board.legal_moves:
        print("lmr pawn search fallback: illegal move")
        _remember_after(board, fallback)
        return fallback
