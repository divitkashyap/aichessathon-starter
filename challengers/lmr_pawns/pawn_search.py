"""Compiled alpha-beta prototype with an isolated pawn-structure evaluator.
...
MAX_PHASE: Final = 24
PASSED_PAWN_EG: Final = np.array([0, 0, 8, 16, 30, 55, 90, 0], dtype=np.int32)
...
def evaluate(board: np.ndarray, state: np.ndarray) -> int:
    ...
    white_bishops = 0
    black_bishops = 0
    pawn_counts = np.zeros((2, 8), dtype=np.int32)
    pawn_maxranks = np.full((2, 8), -1, dtype=np.int32)
    pawn_minranks = np.full((2, 8), 8, dtype=np.int32)
    for square in range(64):
        ...
        if piece == chess.BISHOP:
            ...
            black_bishops += 1
        if piece_type == PAWN:
            side_index = 0 if piece > 0 else 1
            file_index = square & 7
            pawn_rank = square >> 3 if piece > 0 else 7 - (square >> 3)
            pawn_counts[side_index, file_index] += 1
            if pawn_rank > pawn_maxranks[side_index, file_index]:
                pawn_maxranks[side_index, file_index] = pawn_rank
            if pawn_rank < pawn_minranks[side_index, file_index]:
                pawn_minranks[side_index, file_index] = pawn_rank
    if white_bishops >= 2:
        middlegame += 28
        endgame += 38
    if black_bishops >= 2:
    ...
    for side_index in range(2):
        ...
        for file_index in range(8):
            count = int(pawn_counts[side_index, file_index])
            if count == 0:
                continue
            if (file_index == 0 or pawn_counts[side_index, file_index - 1] == 0) and (
                file_index == 7 or pawn_counts[side_index, file_index + 1] == 0
            ):
                middlegame += sign * -10
                endgame += sign * -12
            if count > 1:
                middlegame += sign * -(count - 1) * 10
                endgame += sign * -(count - 1) * 15

            front_rank = int(pawn_maxranks[side_index, file_index])
            passed = True
            for adjacent_file in range(max(0, file_index - 1), min(8, file_index + 2)):
                enemy_minrank = int(pawn_minranks[1 - side_index, adjacent_file])
                if (
                    pawn_counts[1 - side_index, adjacent_file] > 0
                    and enemy_minrank < 7 - front_rank
                ):
                    passed = False
                    break
            if passed:
                endgame_bonus = int(PASSED_PAWN_EG[front_rank])
                middlegame += sign * (endgame_bonus // 2)
                endgame += sign * endgame_bonus
