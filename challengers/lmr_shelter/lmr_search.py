...
@njit(cache=False)
def _shelter_penalty(board: np.ndarray, square: int, side: int) -> int:
    """Small middlegame penalty for missing pawns in front of a castled king."""
    file_index = square & 7
    rank_index = square >> 3
    relative_rank = rank_index if side > 0 else 7 - rank_index
    if relative_rank > 1 or 2 < file_index < 5:
        return 0
    penalty = 0
    for file in range(max(0, file_index - 1), min(7, file_index + 1) + 1):
        near_rank = rank_index + side
        far_rank = rank_index + 2 * side
        if 0 <= near_rank < 8 and board[near_rank * 8 + file] == side * PAWN:
            continue
        if 0 <= far_rank < 8 and board[far_rank * 8 + file] == side * PAWN:
            penalty += 8
        else:
            penalty += 20
    return penalty
...
def evaluate(board: np.ndarray, state: np.ndarray) -> int:
    ...
    if black_bishops >= 2:
        ...
        endgame -= 38
    if black_queen:
        middlegame -= _shelter_penalty(board, white_king, 1)
    if white_queen:
        middlegame += _shelter_penalty(board, black_king, -1)
