...
try:
    from nnue_blend_core import HASH_KEY, position_from_fen
    from nnue_blend_search import (
        MIN_BLEND_PIECES,
        search_timed as blended_search_timed,
        warm_up as blended_warm_up,
    )
    from nnue_blend_classical_search import (
        search_timed as classical_search_timed,
        warm_up as classical_warm_up,
    )
...
blended_warm_up()
classical_warm_up()
BUILD_ID = "einsteinanium-nnue-blend-switch-isolated"
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    try:
        search = (
            classical_search_timed
            if len(board.piece_map()) < MIN_BLEND_PIECES
            else blended_search_timed
        )
        result = search(fen, time_left_ms, GAME_HISTORY)
