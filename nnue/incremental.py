...
from nnue.features import FEATURES_PER_BUCKET
...
def _apply_move_delta(
    ...
    if forward:
        _add_piece_delta(
            accumulator, feature, moved_piece, source, perspective, king_square, -1
        )
        _add_piece_delta(
            accumulator, feature, placed_piece, target, perspective, king_square, 1
        )
        _add_piece_delta(
    ...
    if move & CASTLE:
        ...
        if forward:
            _add_piece_delta(
                accumulator, feature, rook, rook_source, perspective, king_square, -1
            )
            _add_piece_delta(
                accumulator, feature, rook, rook_target, perspective, king_square, 1
            )
        else:
            _add_piece_delta(
                accumulator, feature, rook, rook_target, perspective, king_square, -1
...
def update_after_make(
    ...
    if king_move and moving_side > 0:
        ...
        _apply_move_delta(
            white, 1, int(king_squares[0]), move, undo, feature, True
        )
    ...
    if king_move and moving_side < 0:
        ...
        _apply_move_delta(
            black, -1, int(king_squares[1]), move, undo, feature, True
        )
...
def update_after_unmake(
    ...
    if king_move and moving_side > 0:
        ...
        _apply_move_delta(
            white, 1, int(king_squares[0]), move, undo, feature, False
        )
    ...
    if king_move and moving_side < 0:
        ...
        _apply_move_delta(
            black, -1, int(king_squares[1]), move, undo, feature, False
        )
...
def initialize_quantized(
    board: np.ndarray,
    weights: QuantizedNNUE,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Convenience wrapper for callers holding a ``QuantizedNNUE`` object."""
    return initialize_accumulators(board, weights.feature, weights.feature_bias)
...
def evaluate_incremental(
    ...
    """Convenience wrapper around the compiled accumulator evaluator."""
